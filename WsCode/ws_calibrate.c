...
void calibrate_pots(void)
{
  ...
  else if ((Oi_sw_cal_shoulder == 1) && (Oi_sw_cal_shoulder_prev == 0))
  {
    pot_val = Get_Analog_Value(Analog_in_shoulder);

#ifdef CALIBRATION_DEBUG
    printf("SET shoulder ");
#endif

    if (Oi_arm_shoulder < (127 - CALIBRATE_ZONE))
    {
      /* shoulder stick is moved down, calibrate the back endpoint */
      WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_SHOULDER_B,
                              " back %d", pot_val, ADDR_SHOULDER_B_LO,
                              ADDR_SHOULDER_B_HI);
    }
    else if (Oi_arm_shoulder > (127 + CALIBRATE_ZONE))
    {
      /* shoulder stick is moved up, calibrate the front endpoint */
      WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_SHOULDER_F,
                              " front %d", pot_val, ADDR_SHOULDER_F_LO,
                              ADDR_SHOULDER_F_HI);
    }
  ...
  else if ((Oi_sw_cal_elbow == 1) && (Oi_sw_cal_elbow_prev == 0))
  {
    pot_val = g_cur_elbow;
    ...
    if (Oi_arm_elbow > (127 + CALIBRATE_ZONE))
    {
      /* elbow stick is moved up, calibrate the bottom endpoint */
      WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ELBOW_BOT,
                              " bottom %d", pot_val, ADDR_ELBOW_BOT_LO,
                              ADDR_ELBOW_BOT_HI);
    }
    else if (Oi_arm_elbow < (127 - CALIBRATE_ZONE))
    {
      /* elbow stick is moved down, calibrate the top endpoint */
      WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ELBOW_TOP,
                              " top %d", pot_val, ADDR_ELBOW_TOP_LO,
                              ADDR_ELBOW_TOP_HI);
    }
  ...
  else if ((Oi_sw_cal_rotate == 1) && (Oi_sw_cal_rotate_prev == 0))
  {
    pot_val = Get_Analog_Value(Analog_in_rotate);

#ifdef CALIBRATION_DEBUG
    printf("SET rotate ");
#endif

    if (Oi_arm_rotate > (127 + CALIBRATE_ZONE))
    {
      /* rotate pad is moved left, calibrate the left endpoint */
      WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ROTATE_L,
                              " left %d", pot_val, ADDR_ROTATE_L_LO,
                              ADDR_ROTATE_L_HI);
    }
    else if (Oi_arm_rotate < (127 - CALIBRATE_ZONE))
    {
      /* rotate pad is moved right, calibrate the right endpoint */
      WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ROTATE_R,
                              " right %d", pot_val, ADDR_ROTATE_R_LO,
                              ADDR_ROTATE_R_HI);
    }
  ...
  else if ((Oi_sw_cal_middles == 1) && (Oi_sw_cal_middles_prev == 0))
#else
#define CALIBRATE_MID_VAL  64
  else if ((Oi_sw_cal_middles >= (CALIBRATE_MID_VAL - 10)) &&
           (Oi_sw_cal_middles <= (CALIBRATE_MID_VAL + 10)))
#endif
  {
    /* button combination for horizontal positions - L2 + R2 + X */
#ifdef CALIBRATION_DEBUG
    printf("SET horizontal ");
#endif

    pot_val = Get_Analog_Value(Analog_in_shoulder);
    WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ARM_HORIZ,
                            " s %d", pot_val, ADDR_SHOULDER_HORIZ_LO,
                            ADDR_SHOULDER_HORIZ_HI);

    pot_val = g_cur_elbow;
    WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ARM_HORIZ,
                            " e %d", pot_val, ADDR_ELBOW_HORIZ_LO,
                            ADDR_ELBOW_HORIZ_HI);

    pot_val = Get_Analog_Value(Analog_in_rotate);
    WRITE_CALIBRATION_16BIT(ADDR_DATA_BITMASK_ARM, CAL_MASK_ARM_HORIZ,
                            " r %d", pot_val, ADDR_ROTATE_HORIZ_LO,
                            ADDR_ROTATE_HORIZ_HI);
...
void display_calibration(void)
{
  ...
  if (Oi_calibrate >= OI_CALIBRATE_JOYSTICKS)
  {
    ...
    {
      display_mode = DISPLAY_SHOULDER;
    }
    else if (Oi_arm_elbow >= (127 + CALIBRATE_ZONE))
    {
      display_mode = DISPLAY_ELBOW;
    }
  }
  else
  {
    ...
    {
      display_mode = DISPLAY_POT_SHOULDER;
    }
    else if (Oi_arm_elbow > (127 + CALIBRATE_ZONE))
    {
      display_mode = DISPLAY_POT_ELBOW;
    }
    else if (Oi_arm_rotate > (127 + CALIBRATE_ZONE))
    {
      display_mode = DISPLAY_POT_ROTATE;
    }
      ...
      ((sg_cal_ctr >= 15) && (sg_cal_ctr <= 20)))
  {
    switch (display_mode)
    {
      ...
      case DISPLAY_SHOULDER:
        display_data = Oi_arm_shoulder;
        break;

      case DISPLAY_ELBOW:
        display_data = Oi_arm_elbow;
        break;
