...
void Process_Data_From_Master_uP(void)
{
  ...
  if (autonomous_mode == AUTO_DISABLED)
  {
    if ((Oi_calibrate <= OI_CALIBRATE_ENCODERS) && (user_display_mode == 0))
    {
      ...
      if (Oi_sw_arm_debug == 1)
      {
        ArmPosType arm_pos;
        arm_pos = get_cur_arm_position();
        printf("Arm: s %3d e %3d r %3d  Horiz s %d e %d r %d\r",
               arm_pos.shoulder, arm_pos.elbow, arm_pos.rotate,
    ...
    else
    {
      ...
      //printf("%5d\r", (int)g_elbow_speed);


      /* Process the arm */
      Oi_arm_elbow = joystick_scaling(Oi_arm_elbow, ELBOW_STICK_MIN,
                                      ELBOW_STICK_MIDDLE, ELBOW_STICK_MAX);
      Oi_arm_shoulder = joystick_scaling(Oi_arm_shoulder, SHOULDER_STICK_MIN,
                                         SHOULDER_STICK_MIDDLE, SHOULDER_STICK_MAX);
      arm_control(0);
